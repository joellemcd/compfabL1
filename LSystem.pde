...
for (char c : current.toCharArray()) {
    if (rules.containsKey(c)) {
        currentIterationBuffer.append(rules.get(c)); // Check for and apply the rule from the hashmap
    } else {
        currentIterationBuffer.append(c); // Keep constants constant if there is no rule set for them
    }
}
